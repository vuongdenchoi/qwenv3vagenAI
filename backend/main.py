...
def format_antigraviti_report(analysis_result: dict) -> str:
    ...
    report += "→ Reply **\"OK\"** to run critique using this context.\n"
...
async def unified_chat(
    ...
    try:
        ...
        if image_bytes:
            ...
            # -------------------------------------------------------------------
            # LUỒNG A: ĐANG TRONG PHASE 1 (NHẬN THÔNG TIN BỐI CẢNH TỪ NGƯỜI DÙNG)
            # -------------------------------------------------------------------
            # -------------------------------------------------------------------
            # LUỒNG A: ĐANG TRONG PHASE 10 (HỎI BỐI CẢNH)
            # -------------------------------------------------------------------
            if phase == 10:
                ...
                if any(kw in _msg_lower for kw in _ai_detect_keywords):
                    ...
                    # Trích xuất bối cảnh từ câu chat của user
                    ...
                    if user_knows:
                        ...
                        # Điền khuyết các trường bị thiếu qua Phase 2A
                ...
                # Chạy Phase 2B (RAG Enrichment + Second Pass)
                ...
                memory_store.set_antigraviti_state(
                    key,
                    phase=12,  # Chuyển sang phase=12 chờ confirm
                    image=image_bytes,
                ...
                reply = (
                    ...
                    f"{report_text}\n\n"
                    "Is the context above correct? \n"
                    "→ Reply **\"OK\"** to display visual critique results using this context.\n"
                    "→ Or reply **\"Edit: [details]\"** to modify the context."
                )
            ...
            # -------------------------------------------------------------------
            # LUỒNG A: ĐANG TRONG PHASE 12 (XÁC NHẬN BỐI CẢNH)
            # -------------------------------------------------------------------
            if phase == 12:
                _msg_lower = msg.lower().strip()
                
                # A1: Xác nhận bối cảnh (OK / Đồng ý)
                if check_is_confirm(msg) or _msg_lower in ["ok", "ok nhé", "ok nha", "đồng ý", "tiếp tục"]:
                    memory_store.add_turn(key, "user", msg)
                ...
                # A2: Yêu cầu điều chỉnh bối cảnh (Chỉnh sửa: [điểm cần sửa] hoặc thô)
                elif "chỉnh sửa" in _msg_lower or "sửa lại" in _msg_lower or "sửa bối cảnh" in _msg_lower or "edit" in _msg_lower or "modify" in _msg_lower:
                    memory_store.add_turn(key, "user", msg)
                    ...
                    # Chạy lại Phase 2B đối chiếu RAG
                    try:
                    ...
                    reply = (
                        ...
                        f"{report_text}\n\n"
                        "Is this adjusted context correct? \n"
                        "→ Reply **\"OK\"** to run critique using this context."
                    )
                    memory_store.add_turn(key, "assistant", reply)
                    return {
                        "type": "chat",
            ...
            # -------------------------------------------------------------------
            # LUỒNG A: ĐANG TRONG PHASE 0 (NHẬN TIN NHẮN CHAT TỰ DO)
            # -------------------------------------------------------------------
            if phase == 0:
                _msg_lower = msg.lower().strip()
                
                # C1: Nếu phát hiện các từ khóa kích hoạt Phân tích bối cảnh (Rubic)
                _context_keywords = [
                    # Vietnamese
                ...
                if any(kw in _msg_lower for kw in _context_keywords):
                    print("[Phase-0] Kích hoạt phân tích bối cảnh Rubic theo yêu cầu.")
                    memory_store.add_turn(key, "user", msg)
                    
                    # Chuyển sang phase 10
                ...
                # C2: Nếu phát hiện các từ khóa yêu cầu Feedback/Nhận xét lại
                _critique_keywords = ["feedback lại", "nhận xét lại", "đánh giá lại", "phân tích lại", "critique lại", "feedback", "nhận xét"]
                # Loại trừ trường hợp trùng với bối cảnh
                if any(kw in _msg_lower for kw in _critique_keywords) and not any(kw in _msg_lower for kw in _context_keywords):
                    print("[Phase-0] Thực hiện lại tính năng feedback thiết kế theo yêu cầu.")
                    memory_store.add_turn(key, "user", msg)
                    ...
                    query_str = ", ".join([str(k) for k in keywords if k]) or "graphic design poster advertisement"
                    print(f"[Phase-0 Feedback] Đang chạy critique với query: '{query_str}'...")
                    ...
                    critique_reply = (
                        f"🔄 I have re-analyzed and evaluated your design!\n\n"
                        f"{compliments_text}"
                        f"Detected **{error_count}** visual critique issues to improve.\n"
                        "You can view highlighted bounding boxes directly on the image."
                    )
                    
                    memory_store.add_turn(key, "assistant", critique_reply)
            ...
            # -------------------------------------------------------------------
            # LUỒNG B: PHÂN LOẠI Ý ĐỊNH BẰNG LLM THÔNG MINH (CHO PHASE 2 & PHASE 3 CHAT TỰ DO)
            # -------------------------------------------------------------------
            turns = memory_store.get_recent_turns(key, limit=10)
            history_messages = [{"role": r, "content": [{"text": t}]} for r, t in turns]

            # --- Keyword Guard RE_ANALYZE (trước LLM) ---
            # Bắt sớm các câu yêu cầu phân tích lại bối cảnh để không phụ thuộc LLM classifier
            _msg_b_lower = msg.lower().strip()
            _reanalyze_kw_b = [
            ...
            if any(kw in _msg_b_lower for kw in _reanalyze_kw_b):
                print(f"[Antigraviti LUỒNG-B Guard] Phát hiện RE_ANALYZE keyword. Chạy Phase 2A+2B...")
                try:
                    _re_ctx = run_antigraviti_phase_2a(image_bytes)
                except Exception as _e:
                    print(f"[RE_ANALYZE Guard B] Phase 2A lỗi: {_e}")
                    _re_ctx = {k: None for k in ["thoi_gian", "dia_diem", "lich_su_xa_hoi", "kinh_te", "van_hoa", "art_style", "ty_le", "chat_lieu"]}
                try:
                    _re_res = run_antigraviti_phase_2b(image_bytes, _re_ctx)
                except Exception as _e:
                    print(f"[RE_ANALYZE Guard B] Phase 2B lỗi: {_e}")
                    _re_res = {"conflicts": [], "coherence_total": 5.0, "rag_references": []}
                    for k in _re_ctx.keys():
                        _re_res[k] = {"analysis": "Unable to analyze the context due to a system error.", "score": 5}
                memory_store.set_antigraviti_state(
                    key,
                    phase=2,
                    image=image_bytes,
                    context=_re_ctx,
                    rag_results=_re_res.get("rag_references", []),
                    coherence_scores={k: _re_res.get(k, {}).get("score", 0) if isinstance(_re_res.get(k), dict) else 0 for k in _re_ctx.keys()},
                    conflicts=_re_res.get("conflicts", []),
                    coherence_total=_re_res.get("coherence_total", 0.0)
                )
                _re_report = format_antigraviti_report(_re_res)
                _re_reply = (
                    "I have automatically scanned the design context from the image! 🔍\n\n"
                    f"{_re_report}\n\n"
                    "Does the context above look correct? \n"
                    "→ Reply **\"OK\"** to display visual critique results using this context.\n"
                    "→ Or reply **\"Edit: [details]\"** to modify the context."
                )
                memory_store.add_turn(key, "user", msg)
                memory_store.add_turn(key, "assistant", _re_reply)
                return {"type": "chat", "reply": _re_reply}
            # --- Hết Keyword Guard ---

            pending_gen_prompt = memory_store.get_pending_generation_prompt(key)
            has_original_image = "Có (Sử dụng chế độ Chỉnh sửa ảnh / Image Edits)" if image_bytes else "Không"
            ...
            system_prompt_intent = (
                ...
                "2. MODIFY_CONTEXT: Use when the user wants to change, edit, correct, or modify the design context/parameters (e.g. 'modify context', 'change context', 'edit context to...', 'edit: ...', or if they explicitly mention context values like 'time is 90s', 'culture is Vietnamese').\n"
                ...
                "5. GENERATE_IMAGE: Use when the user wants to generate/create a new image, draw an image, or edit the existing image using image-to-image/inpaint (e.g., 'create new image', 'draw new', 'edit this image', 'generate new image', 'redraw as...').\n"
            ...
            is_confirming_gen = payload.get("is_confirming_generation", False)
            pay_usage = payload.get("_usage", {})

            print(f"[Antigraviti Unified Routing] intent={intent}, confirming_gen={is_confirming_gen}")
            ...
            if (is_confirming_gen or intent == "AGREE") and pending_gen_prompt:
                ...
                if res.get("success"):
                    ...
                    reply = f"Image generation error: {res.get('error')}"
                    memory_store.add_turn(key, "user", msg)
                    memory_store.add_turn(key, "assistant", reply)
                    return {"type": "chat", "reply": reply}
                ...
                # Lấy bối cảnh và ảnh từ phase 2
                ag_phase, ag_image, ag_context, ag_rag, ag_scores, ag_conflicts, ag_total = memory_store.get_antigraviti_state(key)
                ...
                if ag_image and ag_context:
                    ...
                    # Truyền base64 ảnh về frontend để vẽ bounding box (phòng trường hợp UI mất biến lastUploadedFile)
                    import base64
                    img_b64 = base64.b64encode(ag_image).decode('utf-8')
                    mime = "image/jpeg" if ag_image.startswith(b'\xff\xd8') else "image/png"
                    image_data_url = f"data:{mime};base64,{img_b64}"
            ...
            # 2. Ý định MODIFY_CONTEXT: Sửa lại bối cảnh
                ...
                if has_new_details:
                    ...
                    # Chạy Phase 2B (RAG conflict check)
                    try:
                        analysis_result = run_antigraviti_phase_2b(image_bytes, merged_context)
                    except Exception as e:
                    ...
                    # Yêu cầu: Không chạy lại Phase 3 ở bước này.
                    # Sẽ sử dụng kết quả Phase 3 ngầm lúc upload khi user confirm "OK".
                    ...
                    reply = (
                        ...
                        "→ Reply with **\"OK\"** to proceed with the design critique."
                    )
                    memory_store.add_turn(key, "user", msg)
                    memory_store.add_turn(key, "assistant", reply)
                    return {
                    ...
                    # Không có chi tiết bối cảnh mới nào đi kèm -> Hỏi bối cảnh để sửa
                    ...
                    # Thiết lập phase = 1 để lượt chat tiếp theo của user sẽ được trích xuất bối cảnh
                    memory_store.set_antigraviti_state(key, phase=1, image=image_bytes)
                    memory_store.add_turn(key, "user", msg)
                    memory_store.add_turn(key, "assistant", reply)
                    return {"type": "chat", "reply": reply}

            # 3. Ý định RE_ANALYZE: AI tự quét lại bối cảnh ảnh (không cần user cung cấp bối cảnh)
                ...
                # Cập nhật state với bối cảnh mới, giữ phase=2 (chờ user confirm)
                memory_store.set_antigraviti_state(
                    key,
                    phase=2,
                    image=image_bytes,
                ...
                reanalyze_reply = (
                    ...
                    f"{report_text}\n\n"
                    "Please review the context analysis above. "
                    "If it is correct, reply with **\"OK\"** so that I can proceed with the design critique. "
                    "Alternatively, reply with **\"Modify: [what to change]\"** if you wish to adjust the context."
                )
                memory_store.add_turn(key, "user", msg)
                memory_store.add_turn(key, "assistant", reanalyze_reply)
                return {"type": "chat", "reply": reanalyze_reply}

            # 4. Ý định REQUEST_FEEDBACK: Nhận xét lại (cần user cung cấp bối cảnh)
            ...
            # 5. Ý định GENERATE_IMAGE: Sinh ảnh mới
            elif intent == "GENERATE_IMAGE" and payload.get("generate_image_prompt"):
                gen_prompt = payload.get("generate_image_prompt")
                memory_store.set_pending_generation_prompt(key, gen_prompt)
                
                # Đảm bảo AI luôn hỏi xác nhận thay vì "vui lòng chờ..."
                if "confirm" not in reply.lower() and "?" not in reply:
                    reply += "\n\n*(Type **OK** or **Confirm** to proceed with generating the image!)*"
                
                memory_store.add_turn(key, "user", msg)
                memory_store.add_turn(key, "assistant", reply)
                return {"type": "chat", "reply": reply}

            # 6. Ý định ZOOM: Phóng to
            elif intent == "ZOOM":
...
async def regen_image(
    ...
    memory_store.set_last_analysis(key, result_bytes, {"e": []})
