...
4. Common issues to actively look for (Enhanced Focus):
 ...
 5. Bounding box format: You MUST use your native grounding capability. In your reasoning (`r`), when mentioning the element, you MUST append its bounding box in the format `<box>(ymin,xmin),(ymax,xmax)</box>`. Then, in the `"c"` array, output the coordinates as `[xmin, ymin, xmax, ymax]` normalized from 0 to 1000. Ensure the box tightly wraps the element.
